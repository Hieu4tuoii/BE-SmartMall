...
//                "true: lấy toàn bộ món ăn có trong menu của cửa hàng, false: không lấy món ăn"));
        ...
        //         "Get_All_Product",
        //         "Nếu cần thông tin từ danh sách sản phẩm của cửa hàng để đưa ra tư vấn, hãy gọi hàm này",
        //         null
        // );

        //tool 1: tạo query kèm câu filter truy vấn ( tham số sẽ là query ( bắt buộc ), min_price ( tùy chọn ), max_price ( tùy chọn ), category_id ( tùy chọn ), brand_id ( tùy chọn ))
        ...
        //tool 2: Tạo message tư vấn kèm danh sách product_version_id phù hợp nhất ( tham số: message ( bắt buộc ), mảng product_version_id ( tùy chọn ))
        ...
        FunctionDefinition createMessageFunction = new FunctionDefinition(
                "Product_Consulting",
                "Tạo tư vấn 1-3 sản phẩm phù hợp nhất kèm danh sách product_id của các sản phẩm này sau khi có dữ liệu từ hàm Search_Product. Hàm được kết thúc khi tôi gửi lại \"done\"",
